...
import r.android.graphics.Path;
import r.android.graphics.PointF;
import r.android.util.FloatProperty;
import r.android.util.IntProperty;
import r.android.util.Log;
import r.android.util.PathParser;
import r.android.util.Property;
...
public class PropertyValuesHolder implements Cloneable {
    ...
    public static PropertyValuesHolder ofMultiInt(String propertyName, Path path) {
        IKeyframes keyframes = KeyframeSet.ofPath(path);
     ...
     * {@link r.android.animation.TypeEvaluator} can be used to to evaluate the animated value. The
     ...
     * must be a {@link r.android.animation.BidirectionalTypeConverter} to retrieve the current
     * value.
     *
     * <p><strong>Note:</strong> The Object values are stored as references to the original
     * objects, which means that changes to those objects after this method is called will
     ...
     * {@link r.android.util.Property#get(Object)} function.
    ...
    public static PropertyValuesHolder ofKeyframes(String propertyName, IKeyframes keyframes) {
        if (keyframes instanceof IKeyframes.IntIKeyframes) {
            return new IntPropertyValuesHolder(propertyName, (IKeyframes.IntIKeyframes) keyframes);
        } else if (keyframes instanceof IKeyframes.FloatIKeyframes) {
            return new FloatPropertyValuesHolder(propertyName,
                    (IKeyframes.FloatIKeyframes) keyframes);
        } else {
            PropertyValuesHolder pvh = new PropertyValuesHolder(propertyName);
    ...
    public static PropertyValuesHolder ofKeyframes(Property property, IKeyframes keyframes) {
        if (keyframes instanceof IKeyframes.IntIKeyframes) {
            return new IntPropertyValuesHolder(property, (IKeyframes.IntIKeyframes) keyframes);
        } else if (keyframes instanceof IKeyframes.FloatIKeyframes) {
            return new FloatPropertyValuesHolder(property, (IKeyframes.FloatIKeyframes) keyframes);
        } else {
            PropertyValuesHolder pvh = new PropertyValuesHolder(property);
            pvh.mKeyframes = keyframes;
            pvh.mValueType = keyframes.getType();
     ...
     * @param values One or more values that the animation will animate between.
     */
    public void setIKeyframes(Keyframe... values) {
    ...
    void setupSetterAndGetter(Object target) {
        if (mProperty != null) {
            ...
            try {
                ...
                List<Keyframe> keyframes = mKeyframes.getIKeyframes();
    ...
    public void getPropertyValues(PropertyValues values) {
        ...
        if (mKeyframes instanceof PathKeyframes.FloatIKeyframesBase
                || mKeyframes instanceof PathKeyframes.IntIKeyframesBase
                || (mKeyframes.getIKeyframes() != null && mKeyframes.getIKeyframes().size() > 2)) {
    ...
    static class IntPropertyValuesHolder extends PropertyValuesHolder {
        ...
        private static final HashMap<Class, HashMap<String, String>> sJNISetterPropertyMap =
                new HashMap<Class, HashMap<String, String>>();
         String mJniSetter;
        private IntProperty mIntProperty;

        IKeyframes.IntIKeyframes mIntIKeyframes;
        int mIntAnimatedValue;

        public IntPropertyValuesHolder(String propertyName, IKeyframes.IntIKeyframes keyframes) {
            super(propertyName);
            mValueType = int.class;
            mKeyframes = keyframes;
            mIntIKeyframes = keyframes;
        }

        public IntPropertyValuesHolder(Property property, IKeyframes.IntIKeyframes keyframes) {
            super(property);
            mValueType = int.class;
            mKeyframes = keyframes;
            mIntIKeyframes = keyframes;
        ...
        public void setIntValues(int... values) {
            ...
            mIntIKeyframes = (IKeyframes.IntIKeyframes) mKeyframes;
        }

        @Override
        void calculateValue(float fraction) {
            mIntAnimatedValue = mIntIKeyframes.getIntValue(fraction);
        }
        ...
        public IntPropertyValuesHolder clone() {
            ...
            newPVH.mIntIKeyframes = (IKeyframes.IntIKeyframes) newPVH.mKeyframes;
            return newPVH;
        }
        ...
        void setupSetter(Class targetClass) {
            ...
            synchronized(sJNISetterPropertyMap) {
                HashMap<String, String> propertyMap = sJNISetterPropertyMap.get(targetClass);
                boolean wasInMap = false;
                if (propertyMap != null) {
                    wasInMap = propertyMap.containsKey(mPropertyName);
                    if (wasInMap) {
                        String jniSetter = propertyMap.get(mPropertyName);
                        if (jniSetter != null) {
                ...
                if (!wasInMap) {
                    ...
                    try {
                        mJniSetter = nGetIntMethod(targetClass, mPropertyName);
    ...
    static class FloatPropertyValuesHolder extends PropertyValuesHolder {
        ...
        IKeyframes.FloatIKeyframes mFloatIKeyframes;
        float mFloatAnimatedValue;

        public FloatPropertyValuesHolder(String propertyName, IKeyframes.FloatIKeyframes keyframes) {
            super(propertyName);
            mValueType = float.class;
            mKeyframes = keyframes;
            mFloatIKeyframes = keyframes;
        }

        public FloatPropertyValuesHolder(Property property, IKeyframes.FloatIKeyframes keyframes) {
            super(property);
            mValueType = float.class;
            mKeyframes = keyframes;
            mFloatIKeyframes = keyframes;
        ...
        public void setFloatValues(float... values) {
            ...
            mFloatIKeyframes = (IKeyframes.FloatIKeyframes) mKeyframes;
        }

        @Override
        void calculateValue(float fraction) {
            mFloatAnimatedValue = mFloatIKeyframes.getFloatValue(fraction);
        }
        ...
        public FloatPropertyValuesHolder clone() {
            ...
            newPVH.mFloatIKeyframes = (IKeyframes.FloatIKeyframes) newPVH.mKeyframes;
            return newPVH;
        }
        ...
        void setupSetter(Class targetClass) {
            ...
            synchronized (sJNISetterPropertyMap) {
                ...
                if (!wasInMap) {
                    String methodName = getMethodName("set", mPropertyName);
                    try {
                        mJniSetter = nGetFloatMethod(targetClass, mPropertyName);
                    } catch (NoSuchMethodError e) {
                        // Couldn't find it via JNI - try reflection next. Probably means the method
                        // doesn't exist, or the type is wrong. An error will be logged later if
                        // reflection fails as well.
    ...
    static class MultiFloatValuesHolder extends PropertyValuesHolder {
        ...
        public MultiFloatValuesHolder(String propertyName, TypeConverter converter,
                TypeEvaluator evaluator, IKeyframes keyframes) {
            super(propertyName);
            setConverter(converter);
            mKeyframes = keyframes;
            setEvaluator(evaluator);
    ...
    static private  String nGetIntMethod(Class targetClass, String methodName) {return methodName;}
    static private  String nGetFloatMethod(Class targetClass, String methodName) {return methodName;}
    static private  String nGetMultipleIntMethod(Class targetClass, String methodName,
    ...
    static private void nCallFourIntMethod(Object target,  String methodID, int arg1, int arg2,
            int arg3, int arg4) {}
    static private void nCallMultipleIntMethod(Object target,  String methodID, int[] args) {}
    static private void nCallTwoFloatMethod(Object target,  String methodID, float arg1,
            float arg2) {}
    static private void nCallFourFloatMethod(Object target,  String methodID, float arg1,
            float arg2, float arg3, float arg4) {}
    static private void nCallMultipleFloatMethod(Object target,  String methodID, float[] args) {}
}
